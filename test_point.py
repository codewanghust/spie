...
def parse_inputs():
    ...
    parser.add_argument('-m', '--model-path', dest='model_path', default='/mnt/disk1/dat/lchen63/brain/model/center.hdf5')
    parser.add_argument('-ow', '--offset-width', dest='offset_w', type=int, default=1)
    parser.add_argument('-oh', '--offset-height', dest='offset_h', type=int, default=1)
    parser.add_argument('-oc', '--offset-channel', dest='offset_c', nargs='+', type=int, default=1)
    parser.add_argument('-ws', '--width-size', dest='wsize', type=int, default=13)
    parser.add_argument('-hs', '--height-size', dest='hsize', type=int, default=13)
    parser.add_argument('-cs', '--channel-size', dest='csize', type=int, default=13)
    parser.add_argument('-ps', '--pred-size', dest='psize', type=int, default=1)
...
def main(visualize=False, preprocess=False):
    ...
    net = center_model(patch_size=(13, 13, 13), num_classes=5)
    net.summary()
            ...
            for wi in range(batches_w):
                ...
                for ci in range(batches_c):
                    ...
                    if not np.max(data) == 0 and np.min(data) == 0:
                        ...
                        pred[offset_ph:offset_ph+PSIZE, offset_pw:offset_pw+PSIZE, offset_pc:offset_pc+PSIZE, :] += np.squeeze(pred_patch)
                        if visualize:
