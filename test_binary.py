...
def generate_musk(lists):
    label0_2 = lists[0]
    label0_4 = lists[1]
    label0_1 = lists[2]
    musk = np.zeros(shape = label0_2.shape, dtype = np.int8)
    musk[label0_2 !=0 ] = 2
    musk[label0_4 !=0 ] = 4
    musk[label0_1 !=0 ] = 1
    return musk
            ...
            for wi in range(batches_w):
                ...
                for ci in range(batches_c):
                    ...
                    if not np.max(data) == 0 and np.min(data) == 0:
                        ...
                        musk = generate_musk(pred_patch)

                        pred[offset_ph:offset_ph+PSIZE, offset_pw:offset_pw+PSIZE, offset_pc:offset_pc+PSIZE, :] += np.squeeze(musk)
                        if visualize:
                            pred_patch = np.argmax(np.squeeze(musk), axis=-1)
